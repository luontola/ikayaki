...
\classEvent{On measurement started/ended/paused/aborted}{ProjectEvent about the state change will be fired to all project listeners.}
\classEvent{On measurement subphase started/completed}{MeasurementEvent will be fired to all measurement listeners.}
\classEvent{On strike/dip/volume etc. changed}{The updated transformation matrix will be applied to all measurements and a ProjectEvent about the data change will be fired to all project listeners.}
\classEvent{On project file saved}{ProjectEvent about the file saving will be fired to all project listeners.}
\closeClass

\beginToc
\tocMethod{createCalibrationProject(File)}
\tocMethod{createAFProject(File)}
\tocMethod{createThellierProject(File)}
\tocMethod{createThermalProject(File)}
\tocMethod{createProject(File,Type)}
\tocMethod{loadProject(File)}
\tocMethod{closeProject(Project)}
\tocMethod{Project(File,Type)}
\tocMethod{Project(File,Document)}
\tocMethod{getDocument()}
\tocMethod{save()}
\tocMethod{saveNow()}
\tocMethod{exportToDAT(File)}
\tocMethod{exportToSRM(File)}
\tocMethod{exportToTDT(File)}
\tocMethod{getFile()}
\tocMethod{getType()}
\tocMethod{getState()}
\tocMethod{getName()}
\tocMethod{getTimestamp()}
\tocMethod{getSquid()}
\tocMethod{setSquid(Squid)}
\tocMethod{getProperty(String)}
\tocMethod{setProperty(String,String)}
\tocMethod{getStrike()}
\tocMethod{setStrike(double)}
\tocMethod{getDip()}
\tocMethod{setDip(double)}
\tocMethod{getSampleType()}
\tocMethod{setSampleType(SampleType)}
\tocMethod{getOrientation()}
\tocMethod{setOrientation(boolean)}
\tocMethod{getTransform()}
\tocMethod{updateTransforms()}
\tocMethod{getMass()}
\tocMethod{setMass(double)}
\tocMethod{getVolume()}
\tocMethod{setVolume(double)}
\tocMethod{addProjectListener(ProjectListener)}
\tocMethod{removeProjectListener(ProjectListener)}
\tocMethod{fireProjectEvent(ProjectEvent.Type)}
\tocMethod{addMeasurementListener(MeasurementListener)}
\tocMethod{removeMeasurementListener(MeasurementListener)}
\tocMethod{fireMeasurementEvent(MeasurementStep,MeasurementEvent.Type)}
\closeToc

\beginToc
\tocMethod{addSequence(MeasurementSequence)}
\tocMethod{copySequence(int,int)}
\tocMethod{addStep(MeasurementStep)}
\tocMethod{addStep(int,MeasurementStep)}
\tocMethod{removeStep(int)}
\tocMethod{removeStep(int,int)}
\tocMethod{getSteps()}
\tocMethod{getCompletedSteps()}
\tocMethod{getStep(int)}
\tocMethod{getCurrentStep()}
\tocMethod{getValue(int,MeasurementValue)}
\tocMethod{isDegaussingEnabled()}
\tocMethod{isSequenceEditEnabled()}
\tocMethod{isManualControlEnabled()}
\tocMethod{isAutoStepEnabled()}
\tocMethod{isSingleStepEnabled()}
\tocMethod{isPauseEnabled()}
\tocMethod{isAbortEnabled()}
\tocMethod{doAutoStep()}
\tocMethod{doSingleStep()}
\tocMethod{doPause()}
\tocMethod{doAbort()}
\tocMethod{doManualMove(int)}
\tocMethod{doManualRotate(int)}
\tocMethod{doManualMeasure()}
\tocMethod{doManualDemagZ(double)}
\tocMethod{doManualDemagY(double)}
\closeToc
...
\beginField{orientation}
\fieldDeclaration{private boolean orientation}
\fieldValue{true}
\fieldComment{
...
\fieldValue{new Matrix3d().setIdentity()}
\fieldComment{
...
\fieldValue{new LastExecutor(500, true)}
\fieldComment{
...
\methodThrows{NullPointerException}{if file is null.}
\closeMethod
...
\methodThrows{NullPointerException}{if any of the parameters is null.}
\closeMethod
...
\beginMethod{exportToDAT(File)}
\methodDeclaration{public boolean exportToDAT(File file)}
\methodComment{
	Writes the project to a file in DAT format.
}
\methodParam{file}{the file to save to.}
\methodReturn{true if the file was successfully written, otherwise false.}
\methodThrows{NullPointerException}{if file is null.}
\closeMethod

\beginMethod{exportToSRM(File)}
\methodDeclaration{public boolean exportToSRM(File file)}
\methodComment{
...
\beginMethod{exportToTDT(File)}
\methodDeclaration{public boolean exportToTDT(File file)}
\methodComment{
...
\beginMethod{getOrientation()}
\methodDeclaration{public synchronized boolean getOrientation()}
\methodComment{
	Returns the orientation of the sample.
}
\methodReturn{true if the sample orientation is +Z, or false if it is -Z.}
\closeMethod

\beginMethod{setOrientation(boolean)}
\methodDeclaration{public synchronized void setOrientation(boolean orientation)}
\methodComment{
	Sets the orientation of the sample and calls updateTransforms().
}
\methodParam{orientation}{true if the sample orientation is +Z, or false if it is -Z.}
\closeMethod
...
\beginMethod{addMeasurementListener(MeasurementListener)}
...
\beginMethod{removeMeasurementListener(MeasurementListener)}
...
\methodDeclaration{public synchronized boolean addSequence(MeasurementSequence sequence)}
\methodComment{
...
\methodReturn{true if the steps were added, or false if isSequenceEditEnabled() was false.}
...
\methodDeclaration{public synchronized boolean addStep(MeasurementStep step)}
\methodComment{
...
\methodReturn{true if the step was added, or false if isSequenceEditEnabled() was false.}
\methodThrows{NullPointerException}{if step is null.}
\closeMethod

\beginMethod{addStep(int,MeasurementStep)}
\methodDeclaration{public synchronized boolean addStep(int index, MeasurementStep step)}
\methodComment{
...
\methodDeclaration{public synchronized boolean removeStep(int index)}
\methodComment{
...
\methodReturn{true if the step was removed, or false if isSequenceEditEnabled() was false.}
\methodThrows{IndexOutOfBoundsException}{if the index is out of range (index < getCompletedSteps() || index >= getSteps()).}
\closeMethod

\beginMethod{removeStep(int,int)}
\methodDeclaration{public synchronized boolean removeStep(int start, int end)}
\methodComment{
...
\methodReturn{true if the steps were removed, or false if isSequenceEditEnabled() was false.}
...
\beginMethod{doManualMove(int)}
\methodDeclaration{public synchronized boolean doManualMove(int position)}
\methodComment{
...
\methodParam{position}{the position to move the handler to.}
\methodReturn{true if the operation was started, otherwise false.}
\closeMethod

\beginMethod{doManualRotate(int)}
\methodDeclaration{public synchronized boolean doManualRotate(int angle)}
\methodComment{
	Rotates the sample handler to the specified angle. Will do nothing if isManualControlEnabled() is false.
	
	The operation will run in its own thread, and this method will not wait for it to finish.
}
\methodParam{angle}{the angle to rotate the handler to.}
\methodReturn{true if the operation was started, otherwise false.}
\closeMethod

\beginMethod{doManualMeasure()}
\methodDeclaration{public synchronized boolean doManualMeasure()}
\methodComment{
...
\beginMethod{doManualDemagZ(double)}
\methodDeclaration{public synchronized boolean doManualDemagZ(double amplitude)}
\methodComment{
	Demagnetizes the sample in Z direction with the specified amplitude. Will do nothing if isManualControlEnabled() is false.
	
	The operation will run in its own thread, and this method will not wait for it to finish.
}
\methodParam{amplitude}{the amplitude to demagnetize in mT.}
\methodReturn{true if the operation was started, otherwise false.}
\closeMethod

\beginMethod{doManualDemagY(double)}
\methodDeclaration{public synchronized boolean doManualDemagY(double amplitude)}
\methodComment{
...
\beginToc
\tocMethod{MeasurementSequence()}
\tocMethod{MeasurementSequence(String)}
\tocMethod{MeasurementSequence(Element)}
\tocMethod{MeasurementSequence(Element,Project)}
\tocMethod{getElement()}
\tocMethod{getName()}
\tocMethod{setName(String)}
\tocMethod{getSteps()}
\tocMethod{getStep(int)}
\tocMethod{addStep(MeasurementStep)}
\tocMethod{addStep(int,MeasurementStep)}
...
\beginField{steps}
\fieldDeclaration{private List<MeasurementStep> steps}
...
\methodReturn{the created sequence.}
\methodThrows{NullPointerException}{if import is null.}
...
\beginToc
\tocMethod{MeasurementStep()}
\tocMethod{MeasurementStep(Project)}
\tocMethod{MeasurementStep(Element)}
\tocMethod{MeasurementStep(Element,Project)}
\tocMethod{getElement()}
\tocMethod{getProject()}
\tocMethod{getState()}
\tocMethod{setState}
\tocMethod{getTimestamp()}
\tocMethod{getStepValue()}
\tocMethod{setStepValue(double)}
\tocMethod{getMass()}
\tocMethod{setMass(double)}
\tocMethod{getVolume()}
\tocMethod{setVolume(double)}
\tocMethod{updateTransforms()}
\tocMethod{getResults()}
\tocMethod{getResult(int)}
\tocMethod{addResult(MeasurementResult)}
\closeToc
...
\beginToc
\tocMethod{MeasurementResult(Type,double,double,double)}
\tocMethod{MeasurementResult(Element)}
\tocMethod{getElement()}
\tocMethod{setTransform(Matrix3d)}
\tocMethod{getType()}
\tocMethod{getX()}
\tocMethod{getY()}
\tocMethod{getZ()}
...
\methodParam{transform}{the matrix to be applied. If null, will assume identity matrix.}
\closeMethod
...
\beginToc
\tocMethod{getName()}
\tocMethod{rotate(Tuple3d)}
\tocMethod{rotate(Tuple3d,Tuple3d)}
\closeToc
...
\beginToc
\tocMethod{MeasurementValue(String,String,String)}
\tocMethod{getValue(MeasurementStep)}
\tocMethod{getCaption()}
\tocMethod{getUnit()}
\tocMethod{getDescription()}
\closeToc
...
\beginToc
\tocMethod{ProjectEvent(Project,Type)}
\tocMethod{getProject()}
\tocMethod{getType()}
\closeToc
...
\beginToc
\tocMethod{projectUpdated(ProjectEvent)}
\closeToc
...
\beginToc
\tocMethod{MeasurementEvent(Project,MeasurementStep,Type)}
\tocMethod{getProject()}
\tocMethod{getStep()}
\tocMethod{getType()}
\closeToc
...
\beginToc
\tocMethod{measurementUpdated(MeasurementEvent)}
\closeToc

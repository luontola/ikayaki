...
\methodParam{file}{path for this project file. The file should be the same from which import data was read and be writable, but this constructor will not check it.}
\methodParam{import}{import data from which this project will be created.}
\methodReturn{the created project.}
\methodThrows{IllegalArgumentException}{if the import data was not in the right format.}
\closeMethod

\beginMethod{export()}
\methodDeclaration{public synchronized Element export()}
\methodComment{
...
\methodReturn{true if the operation was completed, otherwise false (in which case nothing was changed).}
\closeMethod
...
\methodDeclaration{public synchronized <A> A getValue(int step, MeasurementValue<A> value)}
\methodComment{
...
\methodParam{value}{the algorithm for calculating the desired value.}
\methodReturn{the value returned by the algorithm.}
\closeMethod
...
\methodComment{
	Begins an auto step measurement. Will do nothing if isAutoStepEnabled() is false.
	
	The measurement will run its own thread, and this method will not wait for it to finish.
}
...
\methodComment{
	Begins a single step measurement. Will do nothing if isSingleStepEnabled() is false.

...
\beginMethod{setAmplitude(int amplitude)}
...
\classUses{Settings}
\classUses{SerialIO}
\classComment{ Controls Sample Handler and sets it up, offering interface for it.}
}
\classEvent{On New IO Message }{reads message and puts it in Buffer}
\closeClass
...
\beginMethod{rotateTo(int angle)}
...
\beginMethod{resetCounter(char axis)}
...
\beginMethod{configure(char axis,char subcommand,char option)}
...
\beginMethod{latchAnalog(char axis)}
...
\beginMethod{latchCounter(char axis)}
...
\beginMethod{getData(char axis, char command, string datavalues)}
\methodDeclaration{public String latchCounter(char axis)}
\methodComment{

...
University of Helsinki at the Computer Science Department. The clients is  
Lauri Pesonen with his assistants Fabio Donadini and Tomas Kohout from the 
Department of Geophysics.

This document serves as a contract between client and the project group about the implemented 
functions of the program. Particulary the use cases and requirements sections of this document 
...
operation in many use cases. The work flow now requires the use of various programs 
in different phases of the measurement, which is quite complicated and slow. 
Functions of those two program could be combined to one. The present programs 
are also hard to learn and teach which limits the group of users.

User of the device must know clearly what it's state is and which operation 
is being performed at the moment. The device is delicate and expensive, so a 
misfunction of it must be noticed early to prevent any damage. For example faulty 
...
measurement sequences. It's important that these sequences can be performed 
...
which use a standard file format. So the new program has to be able to export these 
formats.
